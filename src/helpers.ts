...
import { etherscanChainConfig, PLUGIN_NAME } from "./constants";
import {
    ApiResponse,
    ChainConfig,
    EtherscanConfig,
...
export async function printSupportedNetworks() {
    ...
    const supportedNetworks = Object.entries(etherscanChainConfig)
        .map(
            ([network, config]) =>
                [network, config.chainId] as [string, number],
        )
        .sort((a, b) => a[1] - b[1]);
...
export const verifyAllowedChains = (etherscanConfig: EtherscanConfig) => {
    ...
    const invalidNetwork = configKeys.find(
        (chain) => !allowedChains.includes(chain),
    );
            ...
            .get(url, (res) => {
                ...
                res.on("end", () => {
                    if (
                        ...
                        reject(
                            `Request failed. status: ${res.statusCode}, response: ${responseData}`,
                        );
                    }
                });
